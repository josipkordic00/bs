...
router.get("/user/:id", async (req, res) => {
  ...
  let arr = await users
    ...
    .toArray();
>>>>>>> 4bbec4b1d1f3ef36a47135f0fb08e09b94c7b1d4
...
async function loadAppointmentsCollection() {
  const client = await mongodb.MongoClient.connect(
    "mongodb+srv://BeautyStudio:1234@cluster0.sjk4cop.mongodb.net/test?retryWrites=true&w=majority",
    {
...
router.get("/login", async (req, res) => {
  ...
  if (docs.length === 0) {
    ...
    res.send({ err: "Ne postoji korisnik sa tim emailom i lozinkom" });
  } else {
    res.send(docs[0]);
  }
});
...
router.post("/register", async (req, res) => {
  ...
  if (arr.length === 1) {
    res.send({ err: "Email vec postoji" });
  }
  await users.insertOne({
    first_name: req.body.first_name,
    last_name: req.body.last_name,
